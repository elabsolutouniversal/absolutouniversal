    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className={`max-w-5xl mx-auto ${profileData.colors.cardBg} rounded-2xl shadow-xl overflow-hidden ${profileData.colors.border} border`}>
        ...
        <div className={`${profileData.colors.primary} py-10 px-8 text-center relative overflow-hidden`}>
          <div className="absolute inset-0 opacity-10 pattern-dots pattern-amber-100 pattern-size-4" />
        ...
        <div className="p-8 md:p-12">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 mb-16">
            ...
            <div className="text-center lg:text-left space-y-5 flex-1">
              <h2 className="text-3xl font-lora-bold text-gray-800 leading-tight">
                Transformación personal a través de <span className="text-brand-medium">{profileData.generalInfo.specialty}</span>
              </h2>
              <p className="text-lg font-lora text-brand-medium leading-relaxed">
              ...
              <div className="flex flex-wrap justify-center lg:justify-start gap-3 mt-6">
                {profileData.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-2 text-sm font-lora-medium bg-amber-50/40/80 text-brand-medium px-4 py-2 rounded-full border border-brand-light/20"
                  >
          ...
          <div className="mb-16">
            <h3 className="text-3xl font-lora-bold text-gray-800 mb-10 text-center lg:text-left">
              Mi <span className="text-brand-medium">enfoque terapéutico</span>
            </h3>
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-12">
              {profileData.services.map((service, index) => {
                ...
                return (
                  <div 
                    key={index} 
                    className="bg-white p-7 rounded-xl hover:shadow-lg transition duration-300 border border-gray-100 hover:border-brand-light/30 group"
                  >
                    <div className="flex items-center mb-5">
                      <div className="p-3 bg-yellow-100/60 rounded-xl mr-5 group-hover:bg-amber-100/70 transition duration-300">
                        <IconComponent className="h-8 w-8 text-brand-medium" />
                      </div>
                      <h4 className="text-xl font-lora-bold text-gray-800">{service.title}</h4>
                    </div>
                    <p className="font-lora text-brand-medium leading-relaxed">{service.description}</p>
                    <div className="mt-4">
                      <span className="inline-block px-3 py-1 text-xs font-lora-semibold text-brand-medium bg-yellow-100/60 rounded-full">
                        {service.badge}
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-gradient-to-r from-amber-50/40 to-orange-50/20 p-8 rounded-2xl border border-brand-light/20">
                ...
                {profileData.benefits.map((benefit, index) => (
                  <li key={index} className="flex items-start">
                    <div className="flex-shrink-0 mt-1">
                      <div className="flex items-center justify-center h-6 w-6 rounded-full bg-yellow-100/60 text-brand-medium">
                        <Check className="h-4 w-4" />
                      </div>
                    </div>
                    <span className="ml-3 font-lora text-brand-medium font-lora-medium">{benefit}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="text-center">
            <div className="mb-10">
              ...
              <p className="text-xl font-lora text-brand-medium max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4 mb-6">
              ...
              >
                <WhatsAppIcon className="mr-3 text-xl" />
                <span>WhatsApp</span>
            ...
            <div className="bg-amber-50/40/50 p-4 rounded-lg inline-flex items-center gap-3">
              <div className="flex-shrink-0">
                <div className="h-10 w-10 rounded-full bg-yellow-100/60 flex items-center justify-center">
                  <Check className="h-5 w-5 text-brand-medium" />
                </div>
              </div>
              <div className="text-left">
                <p className="text-sm font-lora text-brand-medium">
                  <span className="text-brand-medium font-lora-bold">Oferta especial:</span> {profileData.offer.text}
                </p>
